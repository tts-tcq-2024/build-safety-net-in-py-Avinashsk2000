...
def process_name(input):
    ...
    filtered_name = filter(filter_non_alphabet, name[1:])
...
def build_soundex_code(name, unique_codes):
    """Build the final Soundex code with the first letter and mapped Soundex codes."""
    return (name[0].upper() + ''.join(map(str, unique_codes)))[:4].ljust(4, '0')
...
def generate_soundex(name):
    ...
    return build_soundex_code(name, unique_codes)
